...
public class CheckerUtils {
    ...
    private static CheckerResult runCompilationChecker(Checker checker, List<SubmissionFile> submissionFiles) {
        if (checker.getCheckerType() != CheckerType.COMPILATION) throw new IllegalArgumentException();
        ...
        try (StandardJavaFileManager fileManager = compiler.getStandardFileManager(diagnostics, null, null)) {
            JavaCompiler.CompilationTask task = compiler.getTask(null, fileManager, diagnostics, null, null, fileManager.getJavaFileObjectsFromStrings(filePaths));
            if (task.call()) {
