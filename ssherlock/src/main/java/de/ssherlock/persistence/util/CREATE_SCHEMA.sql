...
CREATE TYPE system_role AS ENUM ('NOT_VERIFIED' , 'REGISTERED', 'TEACHER', 'ADMINISTRATOR');

CREATE TYPE grade AS ENUM ('1', '2', '3', '4', '5', '6');

CREATE TYPE checker_typ AS ENUM ('COMPILATION','IDENTITY','LINE_WIDTH','USER_DEFINED');
...
CREATE TABLE "user" (
                        ...
                        CONSTRAINT email_address_pattern CHECK (email LIKE '_%@_%._%'));
...
CREATE TABLE submission (
                            ...
                            timestamp_submission TIMESTAMP
                                student_username INTEGER REFERENCES "user"(username) ON DELETE CASCADE,
                            tutor_username INTEGER REFERENCES "user"(username) ON DELETE SET NULL,
                            teacher_id INTEGER REFERENCES "user"(username) ON DELETE SET NULL,
...
CREATE TABLE testate_comment (
                                 ...
                                 FOREIGN KEY (submission_id,file_id) REFERENCES submission_file(submission_id,id) ON DELETE CASCADE,
);
