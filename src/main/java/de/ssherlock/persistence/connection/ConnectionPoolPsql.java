...
/**
 * Application-scoped connection pool class for managing PostgreSQL database connections. This class
 * provides methods to create, borrow, and release database connections.
 *
...
public class ConnectionPoolPsql implements Serializable {
  ...
  public synchronized void init() {
    ...
    logger.log(
        Level.INFO,
        "Created " + connections.size() + " connection to database: " + configuration.getDbName());
  }
    ...
    for (Connection conn : borrowedConnections) {
      if (conn != null) {
        try {
          ...
          logger.log(
              Level.WARNING,
              "Borrowed Connection from Connection Pool could not be rollback successfully.");
        }
        try {
          conn.close();
        } catch (SQLException e) {
          logger.log(
              Level.WARNING, "Borrowed Connection from Connection Pool could not be closed.");
        }
      }
    }
    connections.clear();
    ...
    try {
      conn =
          DriverManager.getConnection(
              "jdbc:postgresql://" + configuration.getDbHost() + "/" + configuration.getDbName(),
  ...
  /** Loads the PostgreSQL database driver. Throws an error if the driver is not found. */
