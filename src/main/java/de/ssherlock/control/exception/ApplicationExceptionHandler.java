...
/**
 * Exception handler for handling exceptions. Extends {@link ExceptionHandlerWrapper} to provide
 * additional exception handling capabilities.
 *
...
public class ApplicationExceptionHandler extends ExceptionHandlerWrapper {
    ...
    /**
     * Handles exceptions by delegating to the custom {@code doHandle} method and then invoking the
     * wrapped ExceptionHandler's handle method.
     */
    @Override
    public void handle() {
    ...
    /**
     * Custom exception handling logic. Subclasses can override this method to provide specific
     * handling for different types of exceptions.
     *
     * @param context The FacesContext associated with the current request.
     */
    protected void doHandle(FacesContext context) {
        Iterator<ExceptionQueuedEvent> unhandledExceptions =
        ...
        } else if (exception instanceof DBUnavailableException) {
            showErrorPage(context, exception, "Database unavailable.");
    ...
    /**
     * Factory class for creating instances of {@code ApplicationExceptionHandler}. Extends {@code
     * ExceptionHandlerFactory} to provide the custom exception handler.
     */
