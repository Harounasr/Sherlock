...
/**
 * Oversees user access permissions by verifying whether the role saved in the current session
 * aligns with the required role for specific actions. If there's a mismatch, the user is promptly
 * notified of their lack of permission to perform the action.
 *
  ...
  public void afterPhase(PhaseEvent event) {
    ...
    if (viewRoot != null) {
      ...
      if (url.startsWith("/view/admin") && !appSession.isAdmin()) {
        logger.info(
            "An unauthorized attempt was made to access an admin site without the necessary rights.");
